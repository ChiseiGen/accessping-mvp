...
const trustDisclaimer =
  "Accuracy note: AccessPing is an automated first pass for common WCAG issues on a public page. It does not replace manual keyboard, screen reader, zoom, content, and assistive technology QA before final sign-off.";
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (y > 96) {
      ...
      page.drawLine({ start: { x: left, y }, end: { x: right, y }, thickness: 1, color: line });
      y -= 20;
      drawText("Accuracy Note", left, 12, bold, ink);
      drawWrapped(trustDisclaimer, left, 90, 9, regular, muted);
    }

    page.drawText("Generated by AccessPing. Automated checks do not replace manual accessibility QA.", {
      x: left,
      y: 36,
      size: 9,
      font: regular,
