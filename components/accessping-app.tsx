...
const scoreExplanation = {
  title: "How the access score is calculated",
  description:
    "The score starts at 100 and subtracts points based on issue severity: critical issues deduct 18 points each, serious issues deduct 10, moderate issues deduct 5, and minor issues deduct 2. The final score reflects how many automated accessibility issues were detected on this page.",
  canProve: "Automated checks found issues that can be validated from public page markup.",
  cannotProve: "Manual keyboard navigation, screen reader behavior, zoom behavior, content review, and assistive technology compatibility require human testing."
};
...
function getReportRiskLabel(score: number, issueCount: number) {
  if (score >= 90 && issueCount <= 1) return "Ready";
  if (score >= 75) return "Needs fixes";
  return "Needs fixes";
}

function getReportStatusLabel(result: ScanResult) {
  if (result.summary.critical > 0) return "Blocked";
  if (result.summary.serious > 0) return "Needs fixes";
  if (result.score < 90) return "Needs fixes";
  return "Ready";
}

function generateClientSummary(result: ScanResult) {
  const domain = getDomain(result.url);
  const status = getReportStatusLabel(result);
  const issueCountText = `${result.issueCount} issue group${result.issueCount === 1 ? "" : "s"}`;
  const topIssue = [...result.issues].sort((a, b) => priorityOrder[a.impact] - priorityOrder[b.impact])[0];

  let summary = `Pre-handoff accessibility check for ${domain}\n`;
  summary += `Status: ${status}\n`;
  summary += `Access score: ${result.score}/100\n`;
        ...
        <section className="results isVisible" aria-live="polite" id="issues" data-scroll-reveal>
          ...
          {scoreStatus ? (
            <article className="auditReport" aria-label="Client-ready accessibility report" data-scroll-reveal>
              ...
              <div className="scoreExplanationNote">
                <div className="scoreExplanationHeader">
                  <span>{scoreExplanation.title}</span>
                </div>
                <p>{scoreExplanation.description}</p>
                <div className="scoreExplanationProof">
                  <div>
                    <strong>Can verify</strong>
                    <p>{scoreExplanation.canProve}</p>
                  </div>
                  <div>
                    <strong>Cannot verify</strong>
                    <p>{scoreExplanation.cannotProve}</p>
                  </div>
                </div>
              </div>
          ...
          <div className="reportCta" id="report-preview" data-scroll-reveal>
            ...
            <div className="ctaActions" aria-label="Planned report actions">
              ...
              <CopyableSummaryPanel result={result} />
...
function CopyableSummaryPanel({ result }: { result: ScanResult | null }) {
  const [copied, setCopied] = useState(false);
  const summaryText = result ? generateClientSummary(result) : "";

  async function handleCopy() {
    try {
      await navigator.clipboard.writeText(summaryText);
      setCopied(true);
      track("Client Summary Copied", {
        score: result?.score,
        issueCount: result?.issueCount
      });
  ...
  return (
    <button type="button" onClick={handleCopy} className="copySummaryButton">
      {copied ? (
        <>
          <Check size={15} />
          Copied!
